...
<article class="card wide">
    ...
    <form method="post"
        action="<?= (BASE_URL ? rtrim(BASE_URL, '/') : '') ?>/messages/destroy"
        onsubmit="return confirm('¿Eliminar este mensaje?');">
        <input type="hidden" name="id" value="<?= (int)$mensaje['id'] ?>" />

        <a class="btn" href="<?= (BASE_URL ? rtrim(BASE_URL, '/') : '') ?>/messages/edit?id=<?= (int)$mensaje['id'] ?>">Editar</a>
        <button type="submit" class="btn danger">Eliminar</button>
        <a class="btn secondary" href="<?= (BASE_URL ? rtrim(BASE_URL, '/') : '') ?>/messages">Volver</a>
    </form>
</article>